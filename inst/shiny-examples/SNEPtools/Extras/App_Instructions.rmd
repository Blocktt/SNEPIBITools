...
SNEPtools is a product developed by Tetra Tech, Inc. designed for the Southeast New England Program (SNEP) to use in calculation of benthic macroinvertebrate metrics and index values for SNEP Region streams using Indices of Biotic Integrity (IBI). For more information, see the Background section.

R is an open source programming language and software environment for statistical computing.

Shiny by RStudio, is an interactive web application that serves as a graphical user interface for SNEPtools.
...
# Background: SNEP Low Gradient IBI (v1; 10/20/2020)
An Index of Biotic Integrity (IBI) was calibrated for the Southeast New England Program (SNEP) for benthic macroinvertebrate samples taken in low gradient streams. The IBI is to be used within the SNEP region which encompasses portions of Massachusetts and Rhode Island (Figure 1). Input metrics for SNEP IBI are shown in Table 2. The new IBI will improve SNEP’s diagnostic ability to identify degradation in biological integrity and water quality. Full documentation of development of the SNEP IBI can be found in Jessup, Stamp, and Block (2020).


### Table 2. Metrics in the SNEP IBI, with scoring formulas. Trend is the direction of metric response with increasing stress. 
```{r table_2}
library(readxl)
library(knitr)
# state directories
